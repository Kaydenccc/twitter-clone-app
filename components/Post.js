...
const Post = ({ post, id }) => {
  ...
  const likePost = async () => {
    if (data) {
      if (hasLiked) {
        await deleteDoc(doc(db, 'posts', id, 'likes', data?.user.id));
      } else {
        await setDoc(doc(db, 'posts', id, 'likes', data?.user.id), {
  ...
  return (
    <div className="flex p-3 cursor-pointer border-b border-gray-200">
      ...
      <div className="w-full">
        ...
        {/* Texts */}
        <p onClick={() => router.push(`/posts/${id}`)} className="text-gray-800 text-[15] sm:text-[16px] mb-2">
          {post?.data()?.text}
        </p>
        {/* Post Image */}
